...
[ -n "$version_line" ] || error_echo "Could not find a 'version:' line in $PUBSPEC." "NO" 1
...
[ "$version_number" != "$version_string" ] || error_echo "Version '$version_string' has no build number (expected name+build)." "NO" 1
[ -n "$build_number" ] || error_echo "Could not parse build number from '$version_string'." "NO" 1
ok "Version: $version_number (build $build_number)"

case "$version_number" in
  *-*) error_echo "Version '$version_number' is a pre-release (contains '-'). Bump to a release version first." "NO" 1 ;;
esac
ok "Version number is a release version"

metadata_changelog="metadata/en-US/changelogs/${build_number}4.txt"
[ -f "$metadata_changelog" ] || error_echo "Missing store changelog: $metadata_changelog" "NO" 1
...
for locale in "${required_changelog_locales[@]}"; do
  ...
  [ -f "$asset_changelog" ] || error_echo "Missing in-app changelog: $asset_changelog" "NO" 1
...
for dir in assets/changelogs/*/; do
  ...
  if ! grep -qF -- "- $asset_entry" "$PUBSPEC"; then
    ...
    [ -n "$last_line" ] || error_echo "No existing 'assets/changelogs/' entries in $PUBSPEC to anchor insertion." "NO" 1
...
[ -z "$(git tag -l "$tag")" ] || error_echo "Tag '$tag' already exists." "NO" 1
ok "Tag '$tag' does not exist yet"

if grep -qE '^[[:space:]]*in_app_purchase(_android)?[[:space:]]*:' "$PUBSPEC"; then
  error_echo "in_app_purchase / in_app_purchase_android must be commented out for a FOSS release." "NO" 1
fi
...
[ "$flutter_status" -eq 0 ] || error_echo "FOSS build failed with status code ${flutter_status}." "NO" $flutter_status
