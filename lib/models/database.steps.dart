// dart format width=80
import 'package:drift/internal/versioned_schema.dart' as i0;
import 'package:drift/drift.dart' as i1;
import 'package:tiefprompt/models/keybinding.dart' as i2;
import 'dart:typed_data' as i3;
import 'package:drift/drift.dart'; // GENERATED BY drift_dev, DO NOT MODIFY.

// ignore_for_file: type=lint,unused_import
//
final class Schema2 extends i0.VersionedSchema {
  Schema2({required super.database}) : super(version: 2);
  @override
  late final List<i1.DatabaseSchemaEntity> entities = [
    scriptModel,
    keybindingMapModel,
    settingsPresetModel,
    keybindingMappingModel,
  ];
  late final Shape0 scriptModel = Shape0(
    source: i0.VersionedTable(
      entityName: 'script_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_0, _column_1, _column_2, _column_3],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape1 keybindingMapModel = Shape1(
    source: i0.VersionedTable(
      entityName: 'keybinding_map_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_0],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape2 settingsPresetModel = Shape2(
    source: i0.VersionedTable(
      entityName: 'settings_preset_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_4,
        _column_3,
        _column_5,
        _column_6,
        _column_7,
        _column_8,
        _column_9,
        _column_10,
        _column_11,
        _column_12,
        _column_13,
        _column_14,
        _column_15,
        _column_16,
        _column_17,
        _column_18,
        _column_19,
        _column_20,
        _column_21,
        _column_22,
        _column_23,
        _column_24,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape3 keybindingMappingModel = Shape3(
    source: i0.VersionedTable(
      entityName: 'keybinding_mapping_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_25,
        _column_26,
        _column_27,
        _column_28,
        _column_29,
        _column_30,
        _column_31,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
}

class Shape0 extends i0.VersionedTable {
  Shape0({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get title =>
      columnsByName['title']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get scriptText =>
      columnsByName['script_text']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<DateTime> get createdAt =>
      columnsByName['created_at']! as i1.GeneratedColumn<DateTime>;
}

i1.GeneratedColumn<int> _column_0(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'id',
      aliasedName,
      false,
      hasAutoIncrement: true,
      type: i1.DriftSqlType.int,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'PRIMARY KEY AUTOINCREMENT',
      ),
    );
i1.GeneratedColumn<String> _column_1(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'title',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
    );
i1.GeneratedColumn<String> _column_2(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'script_text',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
    );
i1.GeneratedColumn<DateTime> _column_3(String aliasedName) =>
    i1.GeneratedColumn<DateTime>(
      'created_at',
      aliasedName,
      false,
      type: i1.DriftSqlType.dateTime,
    );

class Shape1 extends i0.VersionedTable {
  Shape1({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
}

class Shape2 extends i0.VersionedTable {
  Shape2({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get name =>
      columnsByName['name']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<DateTime> get createdAt =>
      columnsByName['created_at']! as i1.GeneratedColumn<DateTime>;
  i1.GeneratedColumn<double> get scrollSpeed =>
      columnsByName['scroll_speed']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<bool> get mirroredX =>
      columnsByName['mirrored_x']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<bool> get mirroredY =>
      columnsByName['mirrored_y']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<double> get fontSize =>
      columnsByName['font_size']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<double> get sideMargin =>
      columnsByName['side_margin']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<String> get fontFamily =>
      columnsByName['font_family']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get alignment =>
      columnsByName['alignment']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<bool> get displayReadingIndicatorBoxes =>
      columnsByName['display_reading_indicator_boxes']!
          as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<double> get readingIndicatorBoxesHeight =>
      columnsByName['reading_indicator_boxes_height']!
          as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<bool> get displayVerticalMarginBoxes =>
      columnsByName['display_vertical_margin_boxes']!
          as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<double> get verticalMarginBoxesHeight =>
      columnsByName['vertical_margin_boxes_height']!
          as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<bool> get verticalMarginBoxesFadeEnabled =>
      columnsByName['vertical_margin_boxes_fade_enabled']!
          as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<double> get verticalMarginBoxesFadeLength =>
      columnsByName['vertical_margin_boxes_fade_length']!
          as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<double> get countdownDuration =>
      columnsByName['countdown_duration']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<String> get themeMode =>
      columnsByName['theme_mode']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get appPrimaryColor =>
      columnsByName['app_primary_color']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get prompterBackgroundColor =>
      columnsByName['prompter_background_color']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get prompterTextColor =>
      columnsByName['prompter_text_color']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<bool> get markdownEnabled =>
      columnsByName['markdown_enabled']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<int> get keybindings =>
      columnsByName['keybindings']! as i1.GeneratedColumn<int>;
}

i1.GeneratedColumn<String> _column_4(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'name',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
    );
i1.GeneratedColumn<double> _column_5(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'scroll_speed',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<bool> _column_6(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'mirrored_x',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("mirrored_x" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<bool> _column_7(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'mirrored_y',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("mirrored_y" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<double> _column_8(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'font_size',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<double> _column_9(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'side_margin',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<String> _column_10(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'font_family',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
    );
i1.GeneratedColumn<String> _column_11(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'alignment',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
    );
i1.GeneratedColumn<bool> _column_12(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'display_reading_indicator_boxes',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("display_reading_indicator_boxes" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<double> _column_13(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'reading_indicator_boxes_height',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<bool> _column_14(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'display_vertical_margin_boxes',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("display_vertical_margin_boxes" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<double> _column_15(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'vertical_margin_boxes_height',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<bool> _column_16(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'vertical_margin_boxes_fade_enabled',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("vertical_margin_boxes_fade_enabled" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<double> _column_17(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'vertical_margin_boxes_fade_length',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<double> _column_18(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'countdown_duration',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
    );
i1.GeneratedColumn<String> _column_19(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'theme_mode',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
    );
i1.GeneratedColumn<int> _column_20(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'app_primary_color',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
    );
i1.GeneratedColumn<int> _column_21(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'prompter_background_color',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
    );
i1.GeneratedColumn<int> _column_22(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'prompter_text_color',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
    );
i1.GeneratedColumn<bool> _column_23(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'markdown_enabled',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("markdown_enabled" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<int> _column_24(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'keybindings',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'REFERENCES keybinding_map_model (id) ON DELETE CASCADE',
      ),
    );

class Shape3 extends i0.VersionedTable {
  Shape3({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get mapId =>
      columnsByName['map_id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get keyId =>
      columnsByName['key_id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<bool> get ctrl =>
      columnsByName['ctrl']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<bool> get shift =>
      columnsByName['shift']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<bool> get alt =>
      columnsByName['alt']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<bool> get meta =>
      columnsByName['meta']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<String> get actionName =>
      columnsByName['action_name']! as i1.GeneratedColumn<String>;
}

i1.GeneratedColumn<int> _column_25(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'map_id',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'REFERENCES keybinding_map_model (id) ON DELETE CASCADE',
      ),
    );
i1.GeneratedColumn<int> _column_26(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'key_id',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
    );
i1.GeneratedColumn<bool> _column_27(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'ctrl',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("ctrl" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<bool> _column_28(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'shift',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("shift" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<bool> _column_29(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'alt',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("alt" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<bool> _column_30(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'meta',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("meta" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<String> _column_31(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'action_name',
      aliasedName,
      false,
      check: () => (i0.VersionedTable.col<String>(
        'action_name',
      )).isIn(i2.KeybindingAction.values.map((e) => e.name)),
      type: i1.DriftSqlType.string,
    );

final class Schema3 extends i0.VersionedSchema {
  Schema3({required super.database}) : super(version: 3);
  @override
  late final List<i1.DatabaseSchemaEntity> entities = [
    scriptModel,
    keybindingMapModel,
    settingsPresetModel,
    keybindingMappingModel,
    appStateModel,
  ];
  late final Shape0 scriptModel = Shape0(
    source: i0.VersionedTable(
      entityName: 'script_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_0, _column_1, _column_2, _column_3],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape1 keybindingMapModel = Shape1(
    source: i0.VersionedTable(
      entityName: 'keybinding_map_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_0],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape2 settingsPresetModel = Shape2(
    source: i0.VersionedTable(
      entityName: 'settings_preset_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_4,
        _column_3,
        _column_5,
        _column_6,
        _column_7,
        _column_8,
        _column_9,
        _column_10,
        _column_11,
        _column_12,
        _column_13,
        _column_14,
        _column_15,
        _column_16,
        _column_17,
        _column_18,
        _column_19,
        _column_20,
        _column_21,
        _column_22,
        _column_23,
        _column_24,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape3 keybindingMappingModel = Shape3(
    source: i0.VersionedTable(
      entityName: 'keybinding_mapping_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_25,
        _column_26,
        _column_27,
        _column_28,
        _column_29,
        _column_30,
        _column_31,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape4 appStateModel = Shape4(
    source: i0.VersionedTable(
      entityName: 'app_state_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_32, _column_33],
      attachedDatabase: database,
    ),
    alias: null,
  );
}

class Shape4 extends i0.VersionedTable {
  Shape4({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<bool> get helpRequestShown =>
      columnsByName['help_request_shown']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<String> get lastSeenVersion =>
      columnsByName['last_seen_version']! as i1.GeneratedColumn<String>;
}

i1.GeneratedColumn<bool> _column_32(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'help_request_shown',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("help_request_shown" IN (0, 1))',
      ),
    );
i1.GeneratedColumn<String> _column_33(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'last_seen_version',
      aliasedName,
      true,
      type: i1.DriftSqlType.string,
    );

final class Schema4 extends i0.VersionedSchema {
  Schema4({required super.database}) : super(version: 4);
  @override
  late final List<i1.DatabaseSchemaEntity> entities = [
    scriptModel,
    keybindingMapModel,
    settingsPresetModel,
    keybindingMappingModel,
    appStateModel,
  ];
  late final Shape5 scriptModel = Shape5(
    source: i0.VersionedTable(
      entityName: 'script_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_1,
        _column_2,
        _column_3,
        _column_34,
        _column_35,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape1 keybindingMapModel = Shape1(
    source: i0.VersionedTable(
      entityName: 'keybinding_map_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_0],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape2 settingsPresetModel = Shape2(
    source: i0.VersionedTable(
      entityName: 'settings_preset_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_4,
        _column_3,
        _column_5,
        _column_6,
        _column_7,
        _column_8,
        _column_9,
        _column_10,
        _column_11,
        _column_12,
        _column_13,
        _column_14,
        _column_15,
        _column_16,
        _column_17,
        _column_18,
        _column_19,
        _column_20,
        _column_21,
        _column_22,
        _column_23,
        _column_24,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape3 keybindingMappingModel = Shape3(
    source: i0.VersionedTable(
      entityName: 'keybinding_mapping_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_0,
        _column_25,
        _column_26,
        _column_27,
        _column_28,
        _column_29,
        _column_30,
        _column_31,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape4 appStateModel = Shape4(
    source: i0.VersionedTable(
      entityName: 'app_state_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_32, _column_33],
      attachedDatabase: database,
    ),
    alias: null,
  );
}

class Shape5 extends i0.VersionedTable {
  Shape5({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get title =>
      columnsByName['title']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get scriptText =>
      columnsByName['script_text']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<DateTime> get createdAt =>
      columnsByName['created_at']! as i1.GeneratedColumn<DateTime>;
  i1.GeneratedColumn<bool> get ephemeral =>
      columnsByName['ephemeral']! as i1.GeneratedColumn<bool>;
  i1.GeneratedColumn<double> get scrollPosition =>
      columnsByName['scroll_position']! as i1.GeneratedColumn<double>;
}

i1.GeneratedColumn<bool> _column_34(String aliasedName) =>
    i1.GeneratedColumn<bool>(
      'ephemeral',
      aliasedName,
      false,
      type: i1.DriftSqlType.bool,
      defaultConstraints: i1.GeneratedColumn.constraintIsAlways(
        'CHECK ("ephemeral" IN (0, 1))',
      ),
      defaultValue: Constant(false),
    );
i1.GeneratedColumn<double> _column_35(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'scroll_position',
      aliasedName,
      true,
      type: i1.DriftSqlType.double,
    );

final class Schema5 extends i0.VersionedSchema {
  Schema5({required super.database}) : super(version: 5);
  @override
  late final List<i1.DatabaseSchemaEntity> entities = [
    scriptModel,
    keybindingMapModel,
    settingsPresetModel,
    keybindingMappingModel,
    appStateModel,
  ];
  late final Shape6 scriptModel = Shape6(
    source: i0.VersionedTable(
      entityName: 'script_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_37,
        _column_38,
        _column_39,
        _column_40,
        _column_41,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape1 keybindingMapModel = Shape1(
    source: i0.VersionedTable(
      entityName: 'keybinding_map_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_36],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape7 settingsPresetModel = Shape7(
    source: i0.VersionedTable(
      entityName: 'settings_preset_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_42,
        _column_39,
        _column_43,
        _column_44,
        _column_45,
        _column_46,
        _column_47,
        _column_48,
        _column_49,
        _column_50,
        _column_51,
        _column_52,
        _column_53,
        _column_54,
        _column_55,
        _column_56,
        _column_57,
        _column_58,
        _column_59,
        _column_60,
        _column_61,
        _column_62,
        _column_63,
        _column_64,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape8 keybindingMappingModel = Shape8(
    source: i0.VersionedTable(
      entityName: 'keybinding_mapping_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_65,
        _column_66,
        _column_67,
        _column_68,
        _column_69,
        _column_70,
        _column_71,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape9 appStateModel = Shape9(
    source: i0.VersionedTable(
      entityName: 'app_state_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_72, _column_73],
      attachedDatabase: database,
    ),
    alias: null,
  );
}

class Shape6 extends i0.VersionedTable {
  Shape6({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get title =>
      columnsByName['title']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get scriptText =>
      columnsByName['script_text']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get createdAt =>
      columnsByName['created_at']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get ephemeral =>
      columnsByName['ephemeral']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<double> get scrollPosition =>
      columnsByName['scroll_position']! as i1.GeneratedColumn<double>;
}

i1.GeneratedColumn<int> _column_36(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'id',
      aliasedName,
      false,
      hasAutoIncrement: true,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL PRIMARY KEY AUTOINCREMENT',
    );
i1.GeneratedColumn<String> _column_37(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'title',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<String> _column_38(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'script_text',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<String> _column_39(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'created_at',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_40(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'ephemeral',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL DEFAULT 0 CHECK (ephemeral IN (0, 1))',
      defaultValue: const i1.CustomExpression('0'),
    );
i1.GeneratedColumn<double> _column_41(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'scroll_position',
      aliasedName,
      true,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NULL',
    );

class Shape7 extends i0.VersionedTable {
  Shape7({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get name =>
      columnsByName['name']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get createdAt =>
      columnsByName['created_at']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<double> get scrollSpeed =>
      columnsByName['scroll_speed']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<int> get mirroredX =>
      columnsByName['mirrored_x']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get mirroredY =>
      columnsByName['mirrored_y']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<double> get fontSize =>
      columnsByName['font_size']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<double> get sideMargin =>
      columnsByName['side_margin']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<String> get fontFamily =>
      columnsByName['font_family']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get alignment =>
      columnsByName['alignment']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get displayReadingIndicatorBoxes =>
      columnsByName['display_reading_indicator_boxes']!
          as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<double> get readingIndicatorBoxesHeight =>
      columnsByName['reading_indicator_boxes_height']!
          as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<int> get displayVerticalMarginBoxes =>
      columnsByName['display_vertical_margin_boxes']!
          as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<double> get verticalMarginBoxesHeight =>
      columnsByName['vertical_margin_boxes_height']!
          as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<int> get verticalMarginBoxesFadeEnabled =>
      columnsByName['vertical_margin_boxes_fade_enabled']!
          as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<double> get verticalMarginBoxesFadeLength =>
      columnsByName['vertical_margin_boxes_fade_length']!
          as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<double> get countdownDuration =>
      columnsByName['countdown_duration']! as i1.GeneratedColumn<double>;
  i1.GeneratedColumn<String> get themeMode =>
      columnsByName['theme_mode']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get appPrimaryColor =>
      columnsByName['app_primary_color']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get prompterBackgroundColor =>
      columnsByName['prompter_background_color']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get prompterTextColor =>
      columnsByName['prompter_text_color']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get markdownEnabled =>
      columnsByName['markdown_enabled']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get showControlButtons =>
      columnsByName['show_control_buttons']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get controlButtonsPosition =>
      columnsByName['control_buttons_position']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get keybindings =>
      columnsByName['keybindings']! as i1.GeneratedColumn<int>;
}

i1.GeneratedColumn<String> _column_42(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'name',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<double> _column_43(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'scroll_speed',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_44(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'mirrored_x',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (mirrored_x IN (0, 1))',
    );
i1.GeneratedColumn<int> _column_45(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'mirrored_y',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (mirrored_y IN (0, 1))',
    );
i1.GeneratedColumn<double> _column_46(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'font_size',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<double> _column_47(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'side_margin',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<String> _column_48(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'font_family',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<String> _column_49(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'alignment',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_50(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'display_reading_indicator_boxes',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints:
          'NOT NULL CHECK (display_reading_indicator_boxes IN (0, 1))',
    );
i1.GeneratedColumn<double> _column_51(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'reading_indicator_boxes_height',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_52(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'display_vertical_margin_boxes',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints:
          'NOT NULL CHECK (display_vertical_margin_boxes IN (0, 1))',
    );
i1.GeneratedColumn<double> _column_53(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'vertical_margin_boxes_height',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_54(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'vertical_margin_boxes_fade_enabled',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints:
          'NOT NULL CHECK (vertical_margin_boxes_fade_enabled IN (0, 1))',
    );
i1.GeneratedColumn<double> _column_55(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'vertical_margin_boxes_fade_length',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<double> _column_56(String aliasedName) =>
    i1.GeneratedColumn<double>(
      'countdown_duration',
      aliasedName,
      false,
      type: i1.DriftSqlType.double,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<String> _column_57(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'theme_mode',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_58(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'app_primary_color',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_59(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'prompter_background_color',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_60(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'prompter_text_color',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_61(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'markdown_enabled',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (markdown_enabled IN (0, 1))',
    );
i1.GeneratedColumn<int> _column_62(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'show_control_buttons',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (show_control_buttons IN (0, 1))',
    );
i1.GeneratedColumn<String> _column_63(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'control_buttons_position',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_64(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'keybindings',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints:
          'NOT NULL REFERENCES keybinding_map_model(id)ON DELETE CASCADE',
    );

class Shape8 extends i0.VersionedTable {
  Shape8({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get mapId =>
      columnsByName['map_id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get keyId =>
      columnsByName['key_id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get ctrl =>
      columnsByName['ctrl']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get shift =>
      columnsByName['shift']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get alt =>
      columnsByName['alt']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get meta =>
      columnsByName['meta']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get actionName =>
      columnsByName['action_name']! as i1.GeneratedColumn<String>;
}

i1.GeneratedColumn<int> _column_65(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'map_id',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints:
          'NOT NULL REFERENCES keybinding_map_model(id)ON DELETE CASCADE',
    );
i1.GeneratedColumn<int> _column_66(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'key_id',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_67(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'ctrl',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (ctrl IN (0, 1))',
    );
i1.GeneratedColumn<int> _column_68(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'shift',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (shift IN (0, 1))',
    );
i1.GeneratedColumn<int> _column_69(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'alt',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (alt IN (0, 1))',
    );
i1.GeneratedColumn<int> _column_70(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'meta',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (meta IN (0, 1))',
    );
i1.GeneratedColumn<String> _column_71(
  String aliasedName,
) => i1.GeneratedColumn<String>(
  'action_name',
  aliasedName,
  false,
  type: i1.DriftSqlType.string,
  $customConstraints:
      'NOT NULL CHECK (action_name IN (\'playPause\', \'scrollUpSmall\', \'scrollDownSmall\', \'scrollUp\', \'scrollDown\', \'pageUp\', \'pageDown\', \'jumpStart\', \'jumpEnd\', \'toggleControls\', \'speedUp\', \'speedDown\', \'fontSizeUp\', \'fontSizeDown\', \'openSettings\', \'saveSettingsFromPrompter\'))',
);

class Shape9 extends i0.VersionedTable {
  Shape9({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get helpRequestShown =>
      columnsByName['help_request_shown']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get lastSeenVersion =>
      columnsByName['last_seen_version']! as i1.GeneratedColumn<String>;
}

i1.GeneratedColumn<int> _column_72(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'help_request_shown',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL CHECK (help_request_shown IN (0, 1))',
    );
i1.GeneratedColumn<String> _column_73(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'last_seen_version',
      aliasedName,
      true,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NULL',
    );

final class Schema6 extends i0.VersionedSchema {
  Schema6({required super.database}) : super(version: 6);
  @override
  late final List<i1.DatabaseSchemaEntity> entities = [
    scriptModel,
    keybindingMapModel,
    settingsPresetModel,
    keybindingMappingModel,
    appStateModel,
  ];
  late final Shape6 scriptModel = Shape6(
    source: i0.VersionedTable(
      entityName: 'script_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_37,
        _column_38,
        _column_39,
        _column_40,
        _column_41,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape1 keybindingMapModel = Shape1(
    source: i0.VersionedTable(
      entityName: 'keybinding_map_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_36],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape7 settingsPresetModel = Shape7(
    source: i0.VersionedTable(
      entityName: 'settings_preset_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_42,
        _column_39,
        _column_43,
        _column_44,
        _column_45,
        _column_46,
        _column_47,
        _column_48,
        _column_49,
        _column_50,
        _column_51,
        _column_52,
        _column_53,
        _column_54,
        _column_55,
        _column_56,
        _column_57,
        _column_58,
        _column_59,
        _column_60,
        _column_61,
        _column_62,
        _column_63,
        _column_64,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape8 keybindingMappingModel = Shape8(
    source: i0.VersionedTable(
      entityName: 'keybinding_mapping_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_65,
        _column_66,
        _column_67,
        _column_68,
        _column_69,
        _column_70,
        _column_71,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape9 appStateModel = Shape9(
    source: i0.VersionedTable(
      entityName: 'app_state_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_72, _column_73],
      attachedDatabase: database,
    ),
    alias: null,
  );
}

final class Schema7 extends i0.VersionedSchema {
  Schema7({required super.database}) : super(version: 7);
  @override
  late final List<i1.DatabaseSchemaEntity> entities = [
    scriptModel,
    keybindingMapModel,
    settingsPresetModel,
    keybindingMappingModel,
    appStateModel,
    customFontModel,
  ];
  late final Shape6 scriptModel = Shape6(
    source: i0.VersionedTable(
      entityName: 'script_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_37,
        _column_38,
        _column_39,
        _column_40,
        _column_41,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape1 keybindingMapModel = Shape1(
    source: i0.VersionedTable(
      entityName: 'keybinding_map_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_36],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape10 settingsPresetModel = Shape10(
    source: i0.VersionedTable(
      entityName: 'settings_preset_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_36, _column_42, _column_39, _column_74, _column_64],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape8 keybindingMappingModel = Shape8(
    source: i0.VersionedTable(
      entityName: 'keybinding_mapping_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_65,
        _column_66,
        _column_67,
        _column_68,
        _column_69,
        _column_70,
        _column_71,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape9 appStateModel = Shape9(
    source: i0.VersionedTable(
      entityName: 'app_state_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [_column_72, _column_73],
      attachedDatabase: database,
    ),
    alias: null,
  );
  late final Shape11 customFontModel = Shape11(
    source: i0.VersionedTable(
      entityName: 'custom_font_model',
      withoutRowId: false,
      isStrict: false,
      tableConstraints: [],
      columns: [
        _column_36,
        _column_75,
        _column_76,
        _column_77,
        _column_78,
        _column_79,
        _column_80,
        _column_81,
        _column_82,
      ],
      attachedDatabase: database,
    ),
    alias: null,
  );
}

class Shape10 extends i0.VersionedTable {
  Shape10({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get name =>
      columnsByName['name']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get createdAt =>
      columnsByName['created_at']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get data =>
      columnsByName['data']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get keybindings =>
      columnsByName['keybindings']! as i1.GeneratedColumn<int>;
}

i1.GeneratedColumn<String> _column_74(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'data',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );

class Shape11 extends i0.VersionedTable {
  Shape11({required super.source, required super.alias}) : super.aliased();
  i1.GeneratedColumn<int> get id =>
      columnsByName['id']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<String> get originalFileName =>
      columnsByName['original_file_name']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<String> get familyName =>
      columnsByName['family_name']! as i1.GeneratedColumn<String>;
  i1.GeneratedColumn<int> get weight =>
      columnsByName['weight']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get isItalic =>
      columnsByName['is_italic']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get isVariableWeight =>
      columnsByName['is_variable_weight']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get weightMin =>
      columnsByName['weight_min']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<int> get weightMax =>
      columnsByName['weight_max']! as i1.GeneratedColumn<int>;
  i1.GeneratedColumn<i3.Uint8List> get data =>
      columnsByName['data']! as i1.GeneratedColumn<i3.Uint8List>;
}

i1.GeneratedColumn<String> _column_75(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'original_file_name',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<String> _column_76(String aliasedName) =>
    i1.GeneratedColumn<String>(
      'family_name',
      aliasedName,
      false,
      type: i1.DriftSqlType.string,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_77(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'weight',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL',
    );
i1.GeneratedColumn<int> _column_78(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'is_italic',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NOT NULL DEFAULT 0 CHECK (is_italic IN (0, 1))',
      defaultValue: const i1.CustomExpression('0'),
    );
i1.GeneratedColumn<int> _column_79(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'is_variable_weight',
      aliasedName,
      false,
      type: i1.DriftSqlType.int,
      $customConstraints:
          'NOT NULL DEFAULT 0 CHECK (is_variable_weight IN (0, 1))',
      defaultValue: const i1.CustomExpression('0'),
    );
i1.GeneratedColumn<int> _column_80(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'weight_min',
      aliasedName,
      true,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NULL',
    );
i1.GeneratedColumn<int> _column_81(String aliasedName) =>
    i1.GeneratedColumn<int>(
      'weight_max',
      aliasedName,
      true,
      type: i1.DriftSqlType.int,
      $customConstraints: 'NULL',
    );
i1.GeneratedColumn<i3.Uint8List> _column_82(String aliasedName) =>
    i1.GeneratedColumn<i3.Uint8List>(
      'data',
      aliasedName,
      false,
      type: i1.DriftSqlType.blob,
      $customConstraints: 'NOT NULL',
    );
i0.MigrationStepWithVersion migrationSteps({
  required Future<void> Function(i1.Migrator m, Schema2 schema) from1To2,
  required Future<void> Function(i1.Migrator m, Schema3 schema) from2To3,
  required Future<void> Function(i1.Migrator m, Schema4 schema) from3To4,
  required Future<void> Function(i1.Migrator m, Schema5 schema) from4To5,
  required Future<void> Function(i1.Migrator m, Schema6 schema) from5To6,
  required Future<void> Function(i1.Migrator m, Schema7 schema) from6To7,
}) {
  return (currentVersion, database) async {
    switch (currentVersion) {
      case 1:
        final schema = Schema2(database: database);
        final migrator = i1.Migrator(database, schema);
        await from1To2(migrator, schema);
        return 2;
      case 2:
        final schema = Schema3(database: database);
        final migrator = i1.Migrator(database, schema);
        await from2To3(migrator, schema);
        return 3;
      case 3:
        final schema = Schema4(database: database);
        final migrator = i1.Migrator(database, schema);
        await from3To4(migrator, schema);
        return 4;
      case 4:
        final schema = Schema5(database: database);
        final migrator = i1.Migrator(database, schema);
        await from4To5(migrator, schema);
        return 5;
      case 5:
        final schema = Schema6(database: database);
        final migrator = i1.Migrator(database, schema);
        await from5To6(migrator, schema);
        return 6;
      case 6:
        final schema = Schema7(database: database);
        final migrator = i1.Migrator(database, schema);
        await from6To7(migrator, schema);
        return 7;
      default:
        throw ArgumentError.value('Unknown migration from $currentVersion');
    }
  };
}

i1.OnUpgrade stepByStep({
  required Future<void> Function(i1.Migrator m, Schema2 schema) from1To2,
  required Future<void> Function(i1.Migrator m, Schema3 schema) from2To3,
  required Future<void> Function(i1.Migrator m, Schema4 schema) from3To4,
  required Future<void> Function(i1.Migrator m, Schema5 schema) from4To5,
  required Future<void> Function(i1.Migrator m, Schema6 schema) from5To6,
  required Future<void> Function(i1.Migrator m, Schema7 schema) from6To7,
}) => i0.VersionedSchema.stepByStepHelper(
  step: migrationSteps(
    from1To2: from1To2,
    from2To3: from2To3,
    from3To4: from3To4,
    from4To5: from4To5,
    from5To6: from5To6,
    from6To7: from6To7,
  ),
);
